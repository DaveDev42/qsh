...
- **Controller 측 writer lease 결합 (M3 Step 6).** live 역방향 등록으로 뜨는 host를 향한 controller 쪽의 `qsh session ...`(value op 6종: open/get/list/read/write/resize/close)는 그 명령을 실행한 CLI 프로세스 자신의 QUIC connection이 아니라, 상주 `qsh listen` 데몬이 target과 유지하는 그 **하나의** reverse connection을 `LOCAL_CONTROL` conduit(`docs/design/protocol.md` §11-3)으로 relay해서 나간다. **대화형 attach(`qsh <name>`)는 M3 Step 6 시점에는 아직 이 경로를 타지 않는다** — `Ops::session_attach`는 이 스텝에서도 forward 전용으로 남고(`crate::ops::session::Ops::connect_target`을 그대로 호출), ticket을 실제로 redeem하는 data 스트림도 `LOCAL_STREAM` conduit이 없어 역방향으로 열 수 없다(그 conduit kind는 M3 Step 7의 산출물). 아래 lease 결합 규칙은 지금 value op에 대해 이미 참이고, Step 7이 대화형 attach를 이 메커니즘에 올릴 때 반드시 그대로 유지해야 하는 불변식으로 미리 적어둔다: 위 항목의 "writer lease를 쥐는 connection이 데몬의 reverse connection에 결합된다"는 target 쪽 서술의 controller 쪽 대응이다 — target이 실제로 보는 유일한 connection은 데몬의 것이므로, **writer lease는 데몬의 connection에 묶이지, lease를 요청한 CLI 프로세스 자체에는 묶이지 않는다.** 그 CLI 프로세스가 죽어도(터미널 종료, `Ctrl-C`, 비정상 종료) 데몬의 reverse connection이 살아 있는 한 lease는 자동 해제되지 않는다 — forward 세션(§5, architecture.md §3)의 "소유 connection이 죽으면 lease가 자동 해제"라는 기대가 reverse 경로에서는 CLI 프로세스 단위가 아니라 데몬 connection 단위로 적용된다는 뜻이다. Step 7 이후 다음 대화형 attach는 평소처럼 steal로 lease를 가져갈 수 있지만, `no_steal`을 쓰는 자동화(§6.3)는 이 상태에서 새 lease를 얻지 못하고 이미 죽은 CLI가 남긴 lease와 충돌하는 `SESSION_CONFLICT`를 본다 — 데몬 자신의 reverse connection이 끊기기 전까지는 이 자동화가 스스로 재시도로 빠져나올 방법이 없다.
