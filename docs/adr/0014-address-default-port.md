...
- M9 구현이 이 ADR이 미뤄 둔 값을 채웠다. 결정 2·9의 해당 문장은 2026-09-24에 그 구현(`d5eef05`·`699af37`)에 맞춰 제자리에서 개정했고 옛 문장은 이 파일에 남기지 않는다(ADR-0013 결과의 관례). 결과에는 포트 범위 불강제 사실을 새 bullet으로 추가했다. 원문은 개정 커밋의 diff가 보존한다.
