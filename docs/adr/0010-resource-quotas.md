...
의도된 결과 하나. `-R` permit은 리스너 소유자 principal 기준으로 예약되므로 TCP 쪽 상대가 누구든 같은 풀을 쓴다. 외부에서 오는 무인증 TCP flood가 그 등록의 64와 소유자의 256을 채울 수 있다. 상한이 아예 없던 상태를 소유자 단위의 유계로 바꾸는 것이 목적이다. 리스너 하나가 소유자의 principal 축을 점유하는 것은 소유자가 `-R` 등록 수와 배치로 감수하는 비용이다. 거부 감사 창의 키는 Step 5에서 `(category, principal)`로 바뀌었다. 무인증 flood가 `quota_tunnels_forward`/`quota_tunnels_principal` 창을 채워도 같은 category의 다른 principal은 자기 첫 행을 받는다. 늘어난 상계는 두 겹으로 묶었다. 닫힌 창은 요약 행을 낸 뒤 맵에서 지우고, category당 principal 창은 64개(`quota::MAX_AUDIT_WINDOW_PRINCIPALS`)까지만 두며 그 위는 category마다 하나인 overflow 창으로 모은다. 그래서 category당 행 상계는 창 주기마다 `2 × (min(principal 수, 64) + 1)`행이고, 프로세스 전체 창 수는 `category 수 × 65`를 넘지 않는다. 행 상계가 category당 2행에서 최대 130행으로 오른 결과, principal을 회전시키는 flood 아래에서는 `max_bytes × (retain + 1)` 안에 남는 감사 이력의 기간이 그만큼 짧아진다. 부피 자체는 여전히 유계다.
