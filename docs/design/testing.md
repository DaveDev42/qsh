...
**M10 — release 프로파일 기능 스모크.** `crates/qsh-cli/tests/release_smoke.rs`가 ROADMAP M10 수용 기준의 감사 개정 항목이 요구하는 네 축(init → trust → `exec --json` 왕복 → PTY 셸 + `~d` detach → `qsh attach` 재부착)을 한 시나리오로 잇는다. 이 스위트의 다른 파일과 달리 `QSH_SMOKE_BIN`으로 **임의의 `qsh` 바이너리**를 구동할 수 있고, 그것이 존재 이유다 — 출고되는 것은 release 빌드인데 CI의 기능 테스트는 전부 dev 프로파일이다. 여기서 "`--release` 프로파일 통합 테스트"는 하네스 자체를 release로 컴파일한다는 뜻이 아니라 **출고되는 release 바이너리를 dev 하네스로 구동한다**는 뜻이다 — 검사 대상은 아티팩트이고, `adversarial_load`·`soak`이 `QSH_LOAD_BIN`으로 쓰는 것과 같은 규율이며, 하네스를 release로 빌드해 봐야 그 아티팩트에 대해서는 아무것도 증명하지 못한다. 변수를 주지 않으면 `CARGO_BIN_EXE_qsh`로 떨어져 PR 게이트에서 debug 축으로 돌고, `QSH_SMOKE_STRICT=1`은 그 fallback을 없애 "출고 바이너리를 인증한다"고 약속한 job이 debug를 인증하고 통과하는 길을 막는다. **이 테스트가 증명하는 것**은 그 바이너리로 네 축이 왕복한다는 것뿐이다 — 서명·공증·static 링크·설치 경로는 각각 M10의 다른 항목이 판정하고, 절대 RSS/fd/echo 수치는 `adversarial_load`·`soak`(L9/L10)의 몫이며, 경로 단절 뒤 resume은 `a_severed_path_is_detected_and_resumed_under_a_live_attach`가 계속 담당한다. PTY 축은 unix 전용이고 그 밖의 플랫폼에서는 init·trust·exec 셋만 돈다.
...
- Soak: 24h 수다스러운 세션(메모리 유계), 100 동시 세션(listener RSS ≤ 30MB idle 목표), 10k connect/disconnect 사이클(fd·세션 누수 0), Linux ASAN/LSAN으로 통합 스위트 1회. **구현(M8 Step 5).** 하나의 시나리오 `crates/qsh-cli/tests/soak.rs`가 두 속도로 돈다 — 짧은 모드(기본 120s/8세션, `QSH_SOAK_*` env로 조절)는 `[profile.load]`로 `load.yml`이 push(main)마다 도는 회귀 감시자이고, 24h/100세션 모드는 `[profile.soak]`(`.config/nextest.toml` — `test-threads=1`, `slow-timeout={period="3600s"}`, `terminate-after` 없음, 실측 확인상 kill하지 않고 SLOW 경고만 반복한다)로 GHA 6h 상한 밖에서 `scripts/soak/run.sh`가 기동하고 사람이 `docs/campaigns/m8-soak.md`에 기록한다 — M2 mobility·adversarial-load 캠페인과 같은 지위, PR을 막지 않는다. 판정식(idle RSS ≤ 30MiB, 세션당 buffer ≤ 8MiB, RSS 추세 < 1MiB/h, listener/self fd 성장 ≤ +2, echo p95 spike 비율 ≤ 10%(`ECHO_SPIKE_FRACTION_MAX`, steady 창 중 bound `max(3 × ramp 직후 baseline p95, 50ms)`를 넘는 창의 비율), TTL reap)은 짧은 모드에서 판단 가능한 축은 테스트가 직접 assert하고, 표본이 많이 필요한 두 축(세션당 buffer의 24h 형태·RSS 추세 회귀선)은 `scripts/soak/summarize.py`(stdlib만, CSV를 읽어 위반 시 exit 1)가 24h CSV로 마저 판단한다 — 두 판정이 같은 임계를 공유하는 것은 CSV 헤더 상수(`SOAK_CSV_HEADER`)가 양쪽에 핀으로 박혀 있어서다. listener fd와 self fd 두 축 모두 steady 구간의 fd 표본을 4등분해 마지막 1/4의 최댓값이 첫 1/4의 최댓값+2를 넘는지로 판단한다(사이클링 "도중"의 증가). boot baseline과 drain idle_end를 직접 비교하지는 않는다 — 그 전체 생애주기 비교에는 첫 세션 open이 lazy하게 올리는 일회성 fd 세트(감사 로그 fd·DNS resolver 소켓·keystore fd·tokio io-driver eventfd/epoll: cold-boot 11 → warm-idle ~21)와 ramp의 세션 open/attach 일회성 비용(런타임 warm-up)이 같이 잡혀, (iii)이 실제로 재는 per-cycle 누수와 대상이 다르기 때문이다. 그래서 boot→idle_end 델타는 listener·self 양 축 모두 soak.rs와 summarize.py에 정보성 줄로만 남고 위반으로 세지 않는다. listener 쪽은 soak.rs가 idle_end 시점의 fd 인벤토리(`open_fd_targets`, `/proc/<pid>/fd` readlink)를 그 정보성 줄에 같이 찍어, 델타가 누수가 아니라 위 lazy 세트임을 `load.yml` 로그에서 바로 확인하게 한다 — summarize.py가 읽는 CSV는 fd 개수만 실어 개수 기반 줄로 남긴다. 위반은 `FD_GROWTH_CLIENT`로 태그되는데, 이는 M7 carryover (iii)(pull마다 새 UDS conduit을 여는 `Ops::connect_target` 경로)가 실측으로 재현됐다는 뜻이며 이 축만은 strict 실패로 다룬다. ramp의 세션 open과 사이클 교체 open은 재시도 가능한(`retryable: true`) `ConnectionFailed`/`Timeout` `OpError`를 1초 간격으로 최대 3회까지 재시도한다(fuzz로 포화된 호스트가 qsh-core 자체의 10초 dial 타임아웃을 이따금 못 맞추는 것을 흡수하기 위해서다 — 그 10초 타임아웃 자체는 건드리지 않는다) — 재시도 횟수는 verdict에 정보성으로만 찍히고, 3회를 모두 쓰고서야 실패로 집계된다. 10k connect/disconnect 사이클은 24h 런의 부수적 결과로 기록만 하고(24h × 10세션/60s ≈ 14,400회로 자연히 넘는다), Linux ASAN/LSAN 통합 스위트 1회는 이 Step의 범위 밖으로 남아 별도 1회 항목이다.
...
**M4 perf-gate 긴장의 해소(M4판 계획 §4.1 #7, M4 Step 7에서 반영 완료 — git 이력).** 구 "PR 게이트 금지"라는 일반 원칙과 `docs/design/protocol.md` §12가 M4 수용 기준으로 요구하는 "포화 터널 + PTY echo p95 < 10ms" CI 조기 도입은 문면상 충돌했다. M4에서 결정된 해소 방향은 두 지표를 분리하는 것이다: **비율 throughput**(raw-quinn 대비 터널 ≥ 80%)은 같은 프로세스·같은 실행의 결정적 측정이므로 runner flake에 노출되지 않는다 — 이 지표는 **acceptance job**(`crates/qsh-testkit/tests/tunnel_throughput.rs`, `ci-ok`가 `needs`로 요구하는 그 job)에서 **strict 게이트**로 돈다. **echo-under-load p95**(포화 터널 아래에서의 PTY echo 지연, `crates/qsh-testkit/tests/tunnel_echo_under_load.rs`)는 공유 runner의 wall-clock 변동에 노출되므로 nightly 추세 기록에 남되, **acceptance job에도 게이트**를 하나 추가했다 — 이 둘 다 **PR 단위 테스트 스위트(일반 `cargo test`/`cargo nextest run`)에는 넣지 않는다.** 이는 L4의 "60초 DoD 이중 게이트"(`reverse_blackout.rs` — `QSH_ACCEPTANCE_SLOW` 하 acceptance job)와 같은 패턴이다: PR마다 태우지 않으면서도 DoD 문구를 실측으로 검증한다.

**PRD §13 '느린 파일·터널 stream이 PTY stream을 block하지 않아야 함'의 판정 근거(M10).** 이 항목은 `docs/ROADMAP.md` M10 수용 기준의 M8 이관 항목이 릴리스 게이트로 미뤄 둔 둘 중 하나다 — v1에 파일 전송 표면이 없어 대역 스트림 대체 하네스가 필요하다는 것이 그 유예의 사유였다. 새 하네스를 만들지 않고 `tunnel_saturated_pty_echo_p95_under_measured_rtt_plus_10ms`(`crates/qsh-testkit/tests/tunnel_echo_under_load.rs`)를 그 항목의 판정 근거로 삼는다. `-L` 터널이 PTY와 같은 연결에 지속적인 벌크를 얹을 수 있는 v1의 유일한 표면이고, `FloodServer`가 그 포워드를 host→client 방향으로 포화시키는 모양이 호스트 자신의 송신 스케줄러에게 `PRIORITY_TUNNEL`과 `PRIORITY_SESSION_DATA`를 중재하게 만드는 모양이라, 대체 하네스로 물을 것을 이미 다 묻고 있다. 새 게이트는 만들지 않는다 — 이 테스트는 `ci.yml`의 `acceptance` job에서 `QSH_ACCEPTANCE_STRICT`로 이미 상시 돈다(위 M4 문단). 지금까지 M4 DoD 4의 계약으로만 인용되던 것을 PRD 항목에도 명시 귀속시키는 것이 이 문단의 전부이고, 테스트 자신의 모듈 doc이 같은 귀속을 적는다.

**덮는 축은 포화(고속) 하나다.** 터널 피어가 느리게 읽어 바이트를 flow control에 세워 두는 저속·역압 축은 이 테스트가 재지 않는다. 포화와 기아는 다른 실패 모드이고, 지금 후자를 붙들고 있는 것은 테스트가 아니라 상수 관계다 — `qsh_transport::endpoint::TUNNEL_STREAM_RECEIVE_WINDOW`(2 MiB)가 `CONNECTION_RECEIVE_WINDOW`(8 MiB)의 4분의 1이라 멈춘 터널 스트림 하나로는 세션 스트림 밑의 연결 창을 비울 수 없다. 넷이 동시에 멈추면 비울 수 있고, 그때 PTY 출력이 실제로 막히는지는 실측된 적이 없다. 그 축은 잔여 위험으로 P1에 남긴다.

**M8 DoD 5 T2(적대적 부하, `crates/qsh-cli/tests/adversarial_load.rs`).** 위 M4 perf-gate와 다른 자리를 쓴다 — acceptance job이 아니라 별도 워크플로 `.github/workflows/load.yml`이다. 근거는 같은 절 위쪽의 "절대 throughput 추세는 여전히 nightly" 규율 그대로다: DoD 5가 재는 RSS(MB)·fd 개수·echo p95(ms)는 비율이 아니라 절대 수치이고, acceptance job은 `ci-ok`의 `needs`에 있어 PR 필수 경로다. `load.yml`은 push(main)·`workflow_dispatch`뿐이고 PR을 막지 않는 회귀 감시자다. idle listener RSS 30 MB는 `docs/PRD.md:286`가 "목표"로, `docs/ROADMAP.md:112`가 soak(DoD 2)의 bound로 적은 수치를 DoD 5가 그대로 재사용한 것이며, 재는 대상은 **release 빌드**의 `qsh serve`다(debug 바이너리는 이 30 MB가 애초에 겨냥한 수치가 아니라 `load_bin()`이 `QSH_LOAD_BIN` 미설정을 조용히 넘기지 않고 패닉한다). T2 이전에는 한 번도 실측된 적이 없었다 — 실패 시 코드가 아니라 이 임계 자체를 의심할 여지가 있다는 뜻이고, 진단은 baseline·peak·idle 세 값을 항상 함께 남긴다. 부하 중 상한(`30 MB + 8 MB × 살아 있는 세션 수`)의 8 MB는 `PRD.md:287`의 세션당 replay buffer 설정값에서 빌린 것이지 실측된 RSS 증분이 아니다. echo 임계도 M4와 다르다 — M4는 절대 10ms 고정이고, T2는 **같은 실행 baseline p95의 3배 또는 50ms 중 큰 값**이다(부하 시나리오가 M4처럼 "포화 터널"을 만드는 협조적 부하가 아니라 적대적 flood라 baseline 자체가 실행마다 달라질 수 있기 때문). T1(`crates/qsh-testkit/tests/quota.rs`의 flood-survival 테스트들)의 echo는 `PipeFactory` 기반이라 "pipe echo"이고, T2가 처음으로 서브프로세스 `qsh serve`의 실제 PTY를 재는 "PTY echo"다 — DoD 5 문면의 PTY echo는 T2의 몫이다. RSS/fd 측정은 `/proc` 기반이라 Linux 전용이고, `/proc`이 없는 플랫폼(macOS)에서는 `QSH_LOAD_STRICT`가 아닌 한 그 시나리오 자체가 skip이다.

**M10 — musl 바이너리의 RSS 주장 범위.** `docs/PRD.md` §13의 idle 30 MB는 gnu 바이너리에만 주장한다. `x86_64-unknown-linux-musl` 빌드는 `tikv-jemallocator` 의존이 `target_env = "gnu"`로 좁혀져 시스템(musl) 할당자를 쓰고, 그 의존이 겨냥한 retention 자체가 glibc arena high-water라서 애초에 대상이 아니다. soak·적대적 부하 회차는 계속 gnu 바이너리로만 돌고, musl 자산이 릴리스 전에 통과하는 것은 `release_smoke_covers_init_trust_exec_pty_detach_and_reattach`의 기능 네 축과 정적 링크 증거뿐이다. musl 바이너리의 idle RSS 수치가 필요해지면 그때 별도 회차를 연다.
...
**M9 — `-D`(ADR-0019, ADR-0020).** L3(testkit 하네스)과 L6(CLI 블랙박스)에 SOCKS5 다이나믹 포워드 스위트가 섰다. `crates/qsh-cli/tests/dynamic_forward.rs`는 `dynamic_forward_stub.rs`를 대체해 실패 경로(비-loopback bind, 잘못된 handshake)와 역방향 route의 왕복 성공(ADR-0020 결정 1)을 CLI 블랙박스로 고정하고, `crates/qsh-testkit/tests/dynamic_loopback.rs`는 정방향 route에서 실제 비-loopback 목적지로 SOCKS5 CONNECT 왕복을 실제 비-loopback 주소를 잡는 in-process 하네스로 구동한다 — CI에서 `qsh_testkit::net_probe`가 못 찾는 라우트는 로컬에서는 사유와 함께 skip하지만 CI에서는 그대로 실패로 다룬다(`net_probe::Gap`, 조용한 skip이 회귀를 숨기지 않도록). `crates/qsh-testkit/tests/dynamic_reverse.rs`는 같은 왕복을 역방향 route(`ReverseHarness`)로 반복해 ADR-0020 결정 1(daemon을 거친 `deny_host_local` 통과)과 결정 3(대화형 `-L`의 역방향 왕복)을 고정한다 — `dash_d_over_reverse_reaches_a_non_loopback_echo`, `dash_d_over_reverse_filters_loopback_and_the_target_sees_no_accept`, `interactive_dash_l_over_reverse_round_trips_at_the_ops_level`, `discovery_fails_closed_on_a_shared_runtime_dir_but_resolves_cleanly_once_isolated`. capability(`dial-filter.v1`) 부재 거절은 두 route 모두 `crates/qsh-core/src/ops/tunnel.rs`의 유닛 테스트 둘이 고정한다 — 이 코드베이스에 capability를 뺀 실 peer가 없어 블랙박스로는 구동할 수 없다(ADR-0019 결정 3): `tunnel_dynamic_without_dial_filter_capability_is_unsupported_and_binds_nothing`, `tunnel_dynamic_over_reverse_without_dial_filter_capability_is_unsupported_and_binds_nothing`. `crates/qsh-cli/tests/socks_curl.rs`는 실 `curl`로 acceptance job에서 `-D`를 인수 테스트로 닫는다. L8의 `parse_socks5` fuzz 타깃(위 표)이 이 스위트들의 sans-IO 파서 짝이다.

**M9 — 사람용 표면.** L6에 등록 완전성 게이트 두 겹이 새로 섰다: `crates/qsh-core/tests/op_registration_completeness.rs`의 `section_2_4_fence_matches_every_implemented_operation_bidirectionally`(`:148`)가 `cli_md_section_2_4_fence`(`:125`)로 CLI.md §2.4 펜스를 파싱해 실제 구현된 모든 operation과 양방향으로 대조하고, `layer_2_every_schema_command_has_all_six_faces`(`:440`)가 `OP_FACES` 표(`:205`)를 구동해 `CLI_V1_SCHEMA_COMMANDS`의 각 항목이 여섯 얼굴(schemars 타입, `CLI_V1_SCHEMA_COMMANDS` 등록, main.rs dispatch, human 렌더러, CLI.md 행, man 항목)을 모두 갖췄는지 증명한다 — JSON 렌더러는 이 여섯 얼굴에 들지 않는다(`OpFace`에 그 field가 없다) — §2.4 펜스는 34종, `CLI_V1_SCHEMA_COMMANDS`는 33종으로 서로 다르며(`session.attach`가 그 차이 하나: stream operation이라 schema 항목이 없다), 이 불일치는 버그가 아니라 위 두 게이트가 각자의 대조 대상을 정확히 지키고 있다는 증거다. `crates/qsh-testkit/src/fixtures.rs`의 `normalize`(`:46`)에 마스크 두 개가 더해졌다 — `"manager"`(`:115`)는 host OS의 실제 서비스 매니저(`launchd`/`systemd`)가 담겨 플랫폼마다 값이 달라지므로 `config_dir`과 같은 "모양만 고정" 방식으로 마스크했고(ROADMAP M9 (g), `docs/CLI.md` §6.18), `"cert_pem"`(`:139`)은 leaf PEM이 매 샌드박스 실행마다 새 키/일련번호로 재생성되기 때문이다(ADR-0013) — 각각 `normalize_masks_the_manager_field_to_its_shape`(`:273`)와 `normalize_replaces_volatile_fields_only`(`:253`)가 고정한다. `crates/qsh-cli/tests/fixtures.rs`의 `REQUIRED_FIXTURES`(`:101`)는 `qsh service` 스텝(bd34c92)에서 54개에서 57개로 늘었고 셋 다 신규 파일이다 — M9 전체로는 pre-M9 baseline `525a2a5`의 43개에서 57개로 늘었다. 기존 fixture는 `capabilities.json` 재생성을 빼면 한 개도 고치지 않았다. 값-보유 golden fixture 규율(위 `:88`)을 M9에 적용하면: `capabilities.json`은 M9 동안 실제로 재생성됐다 — `git log --oneline`으로 확인한 유일한 M9 커밋은 `94ad639`(`dial-filter.v1` capability를 더한 `-D` 스텝, ADR-0019/ADR-0020)이며, 그 diff는 `-D` 스텝의 구현 round와 같은 무게로 리뷰됐다. M7 문단(`:169`)의 "golden fixture는 byte-freeze하지 않는다" 판단은 M9의 신규 코드에도 그대로 이어진다 — `EXPECTED_DOCTOR_CODES`(`crates/qsh-core/src/doctor.rs:112`)는 여전히 동결 set-equality(`:643`)로만 고정되고, doctor golden fixture 자체는 `schema.get`과 같은 이유(환경 의존)로 byte-freeze하지 않는다. `docs/CLI.md:917`과 `:1237`의 "22종"(M8까지의 14종에 M9가 8종을 더한 수 — 범위 (h)의 7종 + `host_pinned_without_address`) 산문은 `crates/qsh-core/tests/doctor_docs.rs`의 `cli_md_prose_doctor_code_count_matches_expected_len`(`:138`)이 고정한다. alias 왕복과 config 이중읽기 게이트(ROADMAP M9 DoD 5)에 더해, `crates/qsh-core/tests/failure_text_discipline.rs`가 M9 문면 표본을 새로 얻었다 — `cli_md_section_6_11_quotes_the_address_port_assumed_notice_verbatim`(`:83`), `cli_md_section_6_13_quotes_the_bind_unavailable_remedy_verbatim`(`:94`), `readme_known_limitations_quotes_the_bind_unavailable_remedy_verbatim`(`:107`), `no_pairing_pin_notice_constant_still_says_asked_for_itself`(`:125`), `each_failure_wording_has_observation_impact_and_next_command`(`:443`), `host_not_found_split_has_observation_impact_and_next_command`(`:663`). `crates/qsh-core/tests/service_docs.rs`는 `docs/deploy/service.md`의 여섯 펜스를 생성기 출력과 바이트 단위로 대조한다 — `doctor_docs.rs`/`acl_docs.rs`와 같은 "문서를 상수에 고정" 규율이되, 상수가 아니라 **생성기가 만든 파일에 맞춰 손으로 쓴 산문**을 고정하는 첫 사례다 — 위 `:165`의 man 페이지 게이트도 생성된 파일을 바이트로 대조하지만 그쪽은 대조 대상 자체가 생성물(`.1` 파일)이지 손으로 쓴 산문이 아니다. man 페이지는 같은 `qsh service` 스텝에서 41종에서 45종으로 늘었으며(M9 전체로는 37종에서 45종, 측정: `ls docs/man/*.1 | wc -l` = 45), 새 게이트 없이 위 `:165`의 기존 `checked_in_man_pages_match_the_generator` 문단이 그대로 지킨다.
