...
**동시성 상한:** 한 연결이 동시에 여는 `TCP_CONNECT` 스트림 수는 연결 전체의 bidi 스트림 상한(`MAX_CONCURRENT_BIDI_STREAMS = 1024`, `crates/qsh-transport/src/endpoint.rs`)이 그대로 묶는다 — 동시 터널 스트림과 그것이 붙잡는 upstream fd는 연결당 1024개로 유계이고, 그 peer는 애초에 mTLS로 pin된 상대다(M1–M4 interim allow-all-pinned). 이보다 좁은 **터널 전용** 할당량(principal별·forward별)은 M5 정책 엔진의 default-deny 어휘가 아니라 **M8 적대적 부하 게이트 범위**였고, M8 Step 3b가 인수했다(`docs/ROADMAP.md` M8 감사 개정 ③, `docs/adr/0010-resource-quotas.md`) — `[serve].max_tunnel_streams_per_principal`(기본 256)과 `[serve].max_tunnel_streams_per_forward`(기본 64, `(principal, host:port)` 축)가 `authorize_and_dial_tunnel`이 dial하기 전에 검사하는 별도 상한이며, 초과는 `RESOURCE_EXHAUSTED`로 거부되고 dial은 0건이다. 구현은 `crates/qsh-core/src/quota.rs`의 `Quotas::reserve_tunnel_stream`.

**이 상한이 덮지 않던 것 — remote forward 리스너 개수.** 위 1024는 *스트림* 상한이고, `RemoteForwardOpen`이 여는 것은 스트림이 아니라 listener다: 한 요청이 control 스트림 위의 요청-응답 한 번으로 처리되고(§ 위 "인가 순서" 문단), 그 결과 host에 TCP listener 하나·fd 하나·`serve_remote_forward` task 하나가 뜬다. `forward.remote`를 가진 principal이 서로 다른 `bind_port`로 `RemoteForwardOpen`을 반복해서 보내는 것은 매번 이 1024 상한과 무관한 새 control 요청-응답이라, 이 스트림 상한은 동시에 열려 있는 listener 수를 묶지 못한다 — 그 갭은 바로 위 문단과 같은 **M8 적대적 부하 게이트 범위**였고, M8 Step 3b가 `[serve].max_remote_forwards_per_principal`(기본 16, principal별)로 닫았다(`docs/ROADMAP.md` M8 감사 개정 ③, `docs/adr/0010-resource-quotas.md`). `authorize_and_bind_remote_forward`가 loopback 강제 뒤·`binder.bind` 직전에 검사하고, 초과는 소켓을 하나도 띄우지 않고 `RESOURCE_EXHAUSTED`로 거부된다. 구현은 `crates/qsh-core/src/quota.rs`의 `Quotas::reserve_remote_forward`.
...
**`RemoteForwardClose`의 인가 순서(`PLAN.md` M5 Step 5):** 소유 축은 `RemoteForwardOpen`을 처리한 connection(`conn_id`)이 아니라 그것을 보낸 **principal**이다 — 그 principal이 다른 connection으로 재접속해도 자신이 연 forward는 여전히 닫을 수 있어야 하기 때문이다(`conn_id`는 resume마다 죽으므로 소유 축으로 쓰기에 부적합하다; connection이 죽었을 때의 정리는 여전히 `conn_id` 축이다 — 아래 참조). 순서는 3단계: ① 모양 검사(위 `forward_id` 문단과 동일한 `INVALID_ARGUMENT` 규율, 아직 어떤 상태도 건드리지 않는다) → ② owner 조회(host의 `forward_id → owner` 등록표를 **읽기만** 한다 — 이 조회는 아무것도 판정하지 않는다; `RemoteForwardOpen` 성공 시 등록되는 owner는 요청한 `(principal, auth_path)`다) → ③ `Authorizer::check(principal, auth_path, Action::ForwardRemote, resource)` + audit — `session.control`의 ownership 게이트(§6.9 아래 참조)와 같은 헬퍼가 두 자리 모두에서 쓰인다. ②에서 알려지지 않은 `forward_id`는 owner 없이 ③에 들어가고, owner 없는 리소스는 scope 판정에서 절대 걸러지지 않는다(`docs/design/architecture.md` §6). 이 둘의 ACL 판정이 완전히 동일하게 나오는 것은 **`forward.remote` 권한 자체가 없는 principal**의 경우뿐이다(F2, M5 Step 5 adversarial review) — 그 principal은 owner가 있든 없든 ②의 결과와 무관하게 principal-일치 단계에서 이미 걸리므로, 실재하지 않는 `forward_id`에 대한 판정이 실재하되 남의 것인 `forward_id`에 대한 판정과 byte-for-byte 동일하게 나온다(존재 오라클 없음: `session_loopback.rs`의 `denied_peer_cannot_learn_whether_a_session_exists`와 같은 원칙이 여기서도 적용된다). `forward.remote`를 **가진** principal에게는 이 둘이 갈린다: 실재하되 남의 owner를 가진 `forward_id`는 `scope = "owned"`가 걸러 ③에서 `Deny`(→ `PERMISSION_DENIED`)지만, 알려지지 않은 `forward_id`는 owner가 없어 scope에 걸러지지 않고 ③을 그대로 통과해, 그 뒤의 remove 단계(아래)에서야 "등록표에 없음"으로 `INVALID_ARGUMENT`(§6.9의 "no such forward_id")를 받는다 — `session.write`/`resize`가 남의 세션엔 `PERMISSION_DENIED`, 존재하지 않는 세션엔 (ACL 통과 뒤 broker 조회에서) `SESSION_NOT_FOUND`를 주는 것과 M3이 이미 받아들인 것과 같은 트레이드오프다. ③을 통과한 뒤에야 listener를 remove+abort한다 — 통과했는데 그 시점에 이미 알려지지 않은 id였다면(경쟁으로 이미 닫힌 경우 등) "닫을 것이 없었다"는 성공으로 답하고, 아무것도 다시 만들지 않는다. 이 principal 축은 §11-3의 localctl 쪽 conduit 소유권(`ControlHub`의 `owner: ConduitId`, 같은 로컬 머신 위 두 CLI 프로세스를 구별하는 문제)과는 별개 계층의 축이다 — 이쪽은 원격 QUIC peer가 누구인지, 저쪽은 그 peer를 대신해 로컬에서 요청한 프로세스가 어느 것인지를 각각 답한다.
...
1. `qsh listen`(controller)이 QUIC listener 실행, `qsh serve --to <controller>`(구 표기 `qsh reverse`)(target)가 평범한 상호 TLS로 dial.
...
**pairing 연결 자체가 상한을 넘는 경우(M8 Step 3b 판정 R2)는 위 F-2 경로와 마찬가지로 §15.5의 표 밖이다.** `Principal::Pairing`으로 분류된 연결이 고정 상한(`MAX_CONCURRENT_PAIRING_CONNECTIONS = 8`)을 넘겨 도착하면, `Server::serve_connection`은 그 표가 다루는 `PairingProof` 판정에 아예 들어가지 않는다 — control 스트림을 받지 않고 proof도 읽지 않은 채, 연결을 `CLOSE_CODE_RESOURCE_EXHAUSTED`(`0x1003`)로 즉시 닫는다. 표의 다른 모든 행과 마찬가지로 이 거부도 non-distinguishing이다: `AUTH_FAILED`(secret 불일치)와 겉모습이 다른 `Error` frame조차 쓰지 않으므로, initiator는 invite가 존재하는지조차 이 거부 하나로는 알 수 없다. 감사에는 `quota_connections_pairing`으로 실체가 남는다(`docs/CLI.md` §6.12).
...
이 절은 forward-host(`qsh serve`) 페어링만 다룬다. `qsh listen`/`qsh serve --to`(구 표기 `qsh reverse`)(역방향 conduit)는 이 evaluator를 attach하지 않으므로 페어링 대상이 아니다 — reverse target은 애초에 자신의 accept loop을 돌리지 않고 controller에 dial해 들어가는 쪽이라, 이 절의 TLS 게이트(§15.1)에 도달할 경로 자체가 없다.
...
**상태: 초안 — 발효 전.** 발효 조건: `docs/history/m8-plan.md` §1의 SC7 운영자 판정 — 외부 보안 리뷰를 지금 예약할지((가)), 예약 없이 freeze만 먼저 진행할지((나)) — 이 기록되는 순간이다(`docs/history/m8-plan.md` §1의 (가)/(나) 두 줄). 판정이 나면 이 절 머리의 상태를 "발효"로 바꾸고 아래 두 칸을 채운다. 이 절의 나머지 내용은 (가)/(나) 어느 쪽이 나든 그대로다 — 둘의 차이는 문면 밖의 후속 조치(리뷰 일정, (나)라면 `PRD.md` SC7 문장의 개정)에만 있다(§16.10).
...
1. SC7 판정(`docs/history/m8-plan.md` §1, (가)/(나))이 기록된다.
