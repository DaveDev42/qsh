...
- **수용 기준 (DoD):** 클린 macOS arm64/x86_64·Linux arm64/x86_64에서 brew/curl 설치 → 동작. Gatekeeper가 notarized 바이너리를 차단하지 않음. musl static 바이너리가 구형 glibc 배포판에서 실행.
  ...
  - **(M8 이관 2026-09-10)** `docs/PRD.md:292`(30분 단절 후 TTL 내 복구)·`:293`(느린 파일·터널 stream이 PTY를 block하지 않음)의 직접 증거는 릴리스 게이트에서 판정한다 — `:293`은 파일 전송 표면이 v1에 없어 대역 스트림 대체 하네스가 필요하고, `:292`는 M3 60초 blackout 게이트의 30배 길이라 acceptance job에 못 들어간다(PLAN.md M8 Step 8 (c)).
