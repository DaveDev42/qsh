...
**현재 위치:** M10(릴리스)까지의 코드·계약·파이프라인 스텝 중 에이전트 몫이 전부 착지했고 P0 MVP는 사람 회차 일곱이 닫힐 때 완료로 선언한다. 아직 선언 전이다. M10판 계획은 `docs/history/m10-plan.md`에 있고 `PLAN.md`는 P1 계획을 사용자가 열 때까지 열린 사람 몫만 들고 있는 자리표시자다. 열린 일곱: M10 DoD 1(클린 네 플랫폼 설치·기능 스모크)·DoD 2(Gatekeeper)·DoD 3(구형 glibc musl)은 셋 다 `docs/campaigns/m10-clean-vm.md`의 회차 표가 채워질 때 닫힌다. 나머지 넷은 M7 DoD 1(SC1 스톱워치 baseline 3회, `docs/campaigns/m7-stopwatch.md`, 예행 1회만 끝났다), M8 DoD 3(실기기 mobility ≥60회, `docs/campaigns/m2-mobility.md`), M8 DoD 4(wire freeze 발효와 독립 리뷰 계약, SC7. M5부터 여섯 번째 이월이고 리드타임은 이미 만료됐다), M9 DoD 1(SC1 재측정 3회, `docs/campaigns/m9-stopwatch.md`, M7 DoD 1에 종속)이고 전부 그대로 열려 있다. M8 DoD 1·2·5는 닫혔다(각각 `docs/campaigns/m8-fuzz.md`, `docs/campaigns/m8-soak.md` run #6 PASS, `docs/campaigns/m8-adversarial-load.md`). M2~M10판 계획은 `docs/history/`에 아홉 파일로 있다.
