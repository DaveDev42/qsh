...
M10 실행 계획 원문이다. 2026-09-25 M10 마감 커밋에서 `PLAN.md`를 자리표시자로 교체하면서 M10 원문을 여기로 옮겼다. 이 뒤로는 새 판정을 덧붙이지 않는다. §1 체크박스는 마감 시점 그대로이고 DoD 4~7이 `[x]`, DoD 1·2·3이 `[ ]`다. 열린 채 넘어간 일곱(M10 DoD 1·2·3과 §0.2의 넷)은 `docs/ROADMAP.md`의 "현재 위치"와 자리표시자 `PLAN.md` §0이 함께 들고 있고 M10 마감 판정은 `docs/ROADMAP.md` M10 절의 마감 노트에 있다. M10이 P0의 마지막 마일스톤이라 이 이관 뒤 `PLAN.md`를 이어받을 다음 마일스톤 계획은 없다. P1 계획을 여는 것은 사용자 결정이다.
...
- [ ] **DoD 1 — 클린 네 플랫폼 설치와 기능 스모크.** macOS arm64/x86_64, Linux arm64/x86_64에서 brew 또는 curl 설치 후 `docs/ROADMAP.md` M10 수용 기준의 2026-08-21 감사 개정 항목이 정의한 기능 스모크 네 축이 통과. 근거: Step 10 회차. 소유: 사람(문서 사전 정의는 에이전트). **사람 몫으로 열린 채 마감한다(2026-09-25).** 기준은 `docs/campaigns/m10-clean-vm.md`에 고정됐고 회차 표는 비어 있다.
- [ ] **DoD 2 — Gatekeeper가 notarized 바이너리를 차단하지 않음.** 조작적 정의는 Step 10 캠페인 문서가 고정한다. 근거: Step 5 + Step 10 회차. 소유: 에이전트(CI) + 사람(회차). **에이전트 몫은 끝났고(`c61111b`·`cf24287`, 시크릿 부재 경로는 dispatch run 36034059265·36035270338로 초록) 사람 몫으로 열린 채 마감한다(2026-09-25).** 시크릿 여섯 등록과 서명·공증이 붙은 첫 태그가 선행이다.
- [ ] **DoD 3 — musl static 바이너리가 구형 glibc 배포판에서 실행.** 근거: Step 4 + Step 10의 구형 glibc 회차. 소유: 에이전트 + 사람. **에이전트 몫은 끝났고(`168e00c`, dispatch run 36020104160·36022180833의 DT_NEEDED 0건과 스모크 통과) 구형 glibc 회차 1건이 사람 몫으로 열린 채 마감한다(2026-09-25).**
- [x] **DoD 4 — release 프로파일 기능 스모크가 태그 전 CI에서 최소 1회.** 감사 개정이 `version --json`을 명시적으로 배제했다. 근거: Step 2(하네스) + Step 3(배선). 소유: 에이전트. **충족(2026-09-25).** `152dd78`·`0350438`, dispatch run 36040085569의 unix 다섯 leg과 Windows leg 전부 초록.
- [x] **DoD 5 — PRD §13 두 항목의 직접 증거.** "30분 단절 후에도 TTL 내 세션 복구"와 "느린 파일·터널 stream이 PTY stream을 block하지 않아야 함"이 각각 테스트 이름이나 회차 기록으로 인용 가능. 근거: Step 9. 소유: 에이전트(dispatch 1회 포함). **충족(2026-09-25).** `c2cb64e`, dispatch run 36033815282와 `tunnel_saturated_pty_echo_p95_under_measured_rtt_plus_10ms`.
- [x] **DoD 6 — threat-model이 M9 사람용 표면을 다룸.** §3 진입점과 §4 위협 표에 여섯 명령이 서고 §0의 유예 문장이 걷힌다. 근거: Step 8. 소유: 에이전트. **충족(2026-09-25).** `7119c91`·`381b42c`.
- [x] **DoD 7 — 마감 공통 절차 1·2.** 구속 문서 태그 대조와 README 동기화. 근거: Step 13. 소유: 에이전트. **충족(2026-09-25).** 절차 1 대조 문장 76건, 절차 2 대조 항목 18개. 수치는 `docs/ROADMAP.md` M10 마감 노트에 있다.
...
**(a)-추기 — Step 5 착지 (2026-09-25, main 세션).** 커밋 둘이다. `c61111b`가 `release.yml`의 build job에 darwin 두 leg용 스텝 여섯을 더했고 `cf24287`가 문서를 손봤다. `release.yml` 변경은 순수 추가라 지워진 줄이 없고 다른 leg은 건드리지 않았다. 스텝 순서는 시크릿 판정, 임시 키체인 import, 서명, 검증, 정적 링크 증거, 기동, 스모크, 공증, 패키징, 키체인 삭제(`if: always()`)다. 시크릿은 `env:`로만 들어가고 `set -x`와 echo가 없으며 p12·p8은 `umask 077`에 `trap EXIT`로 지운다. 판정은 전부-또는-전무 일곱 행이다. 0개면 브랜치는 조용히 건너뛰고 태그는 `Unsigned release` 경고와 함께 초록이다. 1개나 5개면 브랜치는 경고 뒤 건너뛰고 태그는 `::error`로 붉다. 6개면 둘 다 `has-apple=true`다. §4.1 #4 확정. identity는 일곱 번째 시크릿이 아니라 임시 키체인에서 `security find-identity -v -p codesigning`으로 읽은 SHA-1 해시이고 매칭 줄이 정확히 하나여야 하며 CN 말미가 `APPLE_TEAM_ID`와 맞아야 한다. §4.1 #5 확정. 공증은 App Store Connect API key 셋으로 하고 `--keychain-profile`은 기각했다. 검증은 `codesign --verify --strict`에 `-R` 요구(`certificate leaf[subject.OU]`가 team id)와 `codesign -dv 2>&1`의 runtime 플래그 grep을 겹쳤고 공증은 스모크 뒤에 `notarytool submit --wait --timeout 30m`으로 하며 jq가 읽은 status가 `Accepted`가 아니면 `notarytool log`를 찍고 붉힌다. 문서는 `docs/deploy/release-secrets.md` 신설(이름·발급처·회전만, 값은 없다. `HOMEBREW_TAP_DEPLOY_KEY`·`CARGO_REGISTRY_TOKEN`·provenance 절 포함), CLAUDE.md 문서 지도 한 줄, README Install 문단과 Known limitations의 Gatekeeper 문단(`codesign -dv --verbose=4`로 Developer ID와 ad-hoc을 가르고 `spctl -a -vvv -t execute`로 판정을 묻는다), `scripts/install.sh`·`scripts/README.md`의 "M10까지 미서명" 문면 셋, testing.md의 CI 규율 불릿과 "현재 상태" 한 구절이다. rebase는 그 "현재 상태" 줄에서 또 충돌했고 attestation 문장과 `long.yml` 문장이 이미 선 main의 줄에 이 스텝의 구절만 끼워 넣어 풀었다.

리뷰는 opus 두 렌즈였고 변이 검증은 렌즈 A만 맡았다. 여섯 변이 중 넷은 repo 안 검출기가 잡는다. 플래그 하나를 빼면(M1) 개수 판정이 5 of 6으로 닫힌다. `--options runtime`을 빼면(M3) `flags=0x2(adhoc)`가 runtime grep에 걸린다. `2>&1`을 빼면(M5) `codesign -d`가 stderr에만 쓰므로 검증이 서명을 탓하며 닫힌다. 5 of 6 태그를 exit 0으로 바꾸면(M6) 진리표가 붉는다. 개수 판정을 exit code 판정으로 바꾼 M4는 identity 0건은 CN 대조가 엉뚱한 메시지로 잡지만 2건(갱신 인증서가 같은 p12에 든 경우)은 두 줄짜리 hash가 출력으로 새어 나간다. 개수 판정이 막는 실제 구멍이다. 전제는 스크래치패드 아래 임시 키체인으로만 확인했고 로그인 search list는 전후 바이트가 같았다. 서명 스텝만 패키징 뒤로 옮긴 M2는 검증 스텝의 `-R`이 잡는다(ad-hoc 바이너리에 `--verify --strict`만은 exit 0, `-R`은 exit 3). 서명·검증·공증 셋을 통째로 패키징 뒤로 옮기면 어느 것도 못 잡고 태그가 초록인 채 ad-hoc tarball을 낸다. 이건 잔여 위험으로 남기고 `Package archive (unix)` 위에 순서 불변 주석 한 줄을 뒀다. 렌즈 B 지적으로 `release.yml` 주석 머리의 PLAN 스텝 번호를 ROADMAP 앵커로, testing.md가 가리키던 아직 없는 `docs/campaigns/m10-clean-vm.md`를 일반 표현으로, README의 현재형 "서명한다"를 "그렇게 나간 릴리스는 아직 없다"로 고쳤다. 렌즈 A 지적으로 p12 `trap`을 먼저 걸고 키체인 출력을 `create-keychain` 직후에 써서 중간 실패가 키를 남기던 구멍을 막았다. `set-key-partition-list`에 `-s`를, `notarytool log` 대체 경로에 `--team-id`와 null id 가드를 더했으며 team id를 보간해 `***`로 가려지던 오류 문구와 무의미한 `set +x`를 지웠다.

게이트는 actionlint, `shellcheck scripts/install.sh`, yaml 파싱, fmt, xtask 13건, 진리표 일곱 행 전부 초록이다. crate 변경이 없어 WSL 회차는 없다. 키체인 규율은 지켰다. import 스크립트는 `bash -n`과 shellcheck만 통과시켰지 실행하지 않았고 `security import`·실 identity `codesign --sign`·`notarytool submit`은 한 번도 부르지 않았다. 브랜치 dispatch run 36034059265는 darwin 두 leg이 `No Apple signing secrets are configured; skipping codesign and notarization.`을 찍고 import·서명·검증·공증·키체인 삭제 다섯 스텝을 건너뛴 채 여섯 leg 모두 초록이었다. main의 dispatch run 36035270338도 같다. 사람 몫은 시크릿 여섯 등록, 첫 태그에서 서명·공증 경로의 실 통과, 그리고 `notarytool` 왕복 실측이다. 실측 전까지 `--timeout 30m`이 남는다.
...
**(a)-추기 — Step 7 착지 (2026-09-25, main 세션).** 커밋 `b38ed80` 하나다. `Package archive (unix)`가 `${RUNNER_TEMP}/pkg`에 staging을 만들어 바이너리와 `docs/man/`의 45면을 `man/` 아래 평평하게 담는다. `export LC_ALL=C`로 glob 순서를 고정해 bsdtar와 GNU tar가 같은 멤버 목록을 내며 `COPYFILE_DISABLE=1`을 스텝 env에 둔다. tar 직전에 `cmp`로 staging의 바이너리가 빌드 산출물과 같은지 보고 로그에 `archive members: N`과 첫·끝 멤버를 찍는다. 단언은 둘이다. `^qsh$`도 `^man/[^/]*\.1$`도 아닌 멤버가 있으면 `::error title=Archive contents::`로 붉고 아카이브의 man 페이지 수가 `docs/man/*.1`의 수와 다르면 같은 제목으로 붉는다. 두 번째 단언은 리뷰 뒤 main 세션이 더한 것이다. 이 둘은 `Static-link evidence (musl)`과 같은 부류의 워크플로 스텝 단언이지 repo 테스트 게이트가 아니다. windows zip은 그대로다. 문면은 README의 man 문단과 Homebrew 절, `scripts/README.md` 한 문장, `scripts/install.sh`의 주석, `release_smoke.rs` 모듈 문서의 `Binary starts` 표기다. `install.sh`의 로직은 한 바이트도 안 바뀌었다. §4.1 #7 확정.

리뷰는 opus 두 렌즈였고 변이 다섯은 렌즈 A만 맡았다. M1(tar 피연산자에서 `man/*.1` 제거)은 리뷰 시점에 아무 검출기도 없었다. 가드가 낯선 멤버만 거부하고 기대 멤버를 요구하지 않아서다. 그래서 페이지 수 단언을 더했고 같은 변이가 이제 `holds 0 man pages but docs/man/ has 45`로 붉는다. M2(staging에 `LICENSE`를 두기)는 문자 그대로는 무효였다. tar 피연산자가 `qsh man/*.1`로 명시돼 staging의 낯선 파일이 아카이브에 들어갈 길이 없다. 피연산자에 넣는 형태로 바꾸면 가드가 잡는다. M3(`man/man1/*.1` 중첩)은 정규식의 두 번째 슬래시에서 붉는다. M4(`LC_ALL=C` 제거)는 검출기가 없고 주석이 그 값을 적고 있다는 것만 확인했다. M5(formula의 `man1.install` 삭제)는 이 repo 안에 검출기가 없고 클린 VM 캠페인의 brew 회차가 `man qsh`로 본다. 렌즈 A가 잰 사실 하나: 이 bsdtar(libarchive 3.7.4)는 `COPYFILE_DISABLE` 유무와 무관하게 AppleDouble 멤버를 내지 않아 그 변수는 보험이다. 렌즈 B의 major는 문면이었다. README와 `scripts/README.md`가 man 페이지를 현재형으로 말했지만 공개된 0.2.0 아카이브는 멤버가 `qsh` 하나이고 formula에도 `man1.install`이 없어 첫 M10 태그 전에는 거짓이다. attestation 문단이 이미 쓰는 "첫 태그부터" 한정을 같은 모양으로 붙였고 ROADMAP Q4의 문장 "Homebrew installs them; the curl installer does not"는 그대로 뒀다. Windows zip에는 `man/`이 없으므로 그 절은 `.tar.gz`로 한정했다.

게이트 여덟은 첫 회에 초록이었고 actionlint·shellcheck·README 인용 테스트도 초록이다. 로컬 리허설은 실제 YAML에서 스텝 본문을 뽑아 bash 3.2로 돌리는 하네스로 했다. 46멤버, 첫 멤버 `qsh`, 끝 멤버 `man/qsh.1`. `install.sh`는 curl 스텁으로 네 경우를 돌았다. 46멤버 아카이브에서 `qsh`만 꺼내고 symlink 멤버는 거부한다. `man/`만 있는 아카이브는 자기 오류로 죽고 잘못된 SHA256SUMS는 checksum mismatch로 죽는다. Linux 확인은 `dave-windows-wsl`에서 `b38ed80`으로 했다. clippy 초록, `release_smoke` 통과이고, 같은 스텝 본문을 GNU tar 1.35로 돌린 멤버 목록이 bsdtar의 목록과 바이트 단위로 같다.

dispatch run 36040085569은 브랜치에서 돌렸다. build 여섯 leg 전부 초록이고 unix leg 넷의 로그마다 `archive members: 46`과 `qsh`·`man/qsh.1`이 찍혔다. aarch64-apple-darwin 아티팩트를 내려받아 `tar tzf`로 본 46멤버 목록이 로컬 리허설 목록과 같고 낯선 멤버도 `._` 멤버도 없으며, 그 아카이브로 `scripts/install.sh`를 curl 스텁에 물려 돌리면 `qsh` 하나만 설치되고 `qsh --version`이 뜬다. `grep -rn 'M9' scripts/`는 0건이다. tap은 main 세션이 스크래치패드의 새 clone에서 `man1.install Dir["man/*.1"]` 한 줄을 넣어 커밋 0407d57로 push했다. `brew style` 무위반이고 bump job의 awk를 그 formula에 돌리면 version·url·sha256 세 줄만 바뀌고 이 줄은 남는다. 첫 M10 태그 전까지 0.2.0 설치는 `Dir`이 비어 `opoo` 경고 한 줄을 남기고 성공한다.
...
| 4 | 서명 identity 문자열의 출처 | `APPLE_TEAM_ID` 시크릿으로 조립할지 identity 전체를 시크릿에 둘지 **확정(2026-09-25):** 둘 다 아니다. 임시 키체인에 `APPLE_CERT_P12`를 import한 뒤 `security find-identity -v -p codesigning`이 돌려주는 SHA-1 해시를 서명에 쓴다(`c61111b`). 매칭 줄이 정확히 하나여야 하고 CN 말미가 `APPLE_TEAM_ID`와 맞아야 한다. 일곱 번째 시크릿은 두지 않는다. | Step 5 |
