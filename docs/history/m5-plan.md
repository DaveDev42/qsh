# M5 계획 — ACL + audit (감사 기록)

2026-09-24에 복원했다. 아래 본문은 M5 실행 계획 원문이다. 커밋 `9d61b13`(2026-08-28)의 `PLAN.md`에서 그대로 꺼냈고, 같은 날 커밋 `cbf2094`가 `PLAN.md`를 M6 계획으로 교체하면서 이 판본은 저장소 작업 트리에서 사라졌다. 새 판정을 덧붙이지 않고 원문 그대로 둔다. 복원 사유는 코드 주석의 옛 계획 인용이 가리키는 문서를 저장소 안에 다시 두기 위함이다.
...
**검증 라운드 확정 — Step 8 adversarial (2026-08-28, main 세션 판정).** P1 1건·P2 7건 전건 처분: ① P1-1(testing.md 신규 문장이 "항상-deny 3종을 포함한 14개 seam"으로 사실 반전 — DENY_SEAMS 14행은 3종을 **제외**한 수) 수용, 문장 정정. ② P2-1(`OpSpec::owned`·`resource_kind`가 읽는 곳 없는 죽은 데이터 — 필드 반전 mutation이 24/24 생존) 수용하되 삭제가 아니라 결속으로: PLAN (a)가 두 필드를 명시했으므로 DoD 2 구동 하네스가 `resource_kind`를 소비해 resource 문자열을 만들고, `owned` 열은 비소유 principal + `scope="owned"` 정책 구동으로 행동과 대조한다. ③ P2-2(DoD 2의 action 단언은 registry 동어반복 — 다만 dispatch↔`action_of`↔registry↔CLI.md의 사슬은 mutation으로 끊김 없음 실증) 수용, 모듈 doc에 한계 1줄 명기로 종결. ④ P2-3(`action_of` panic이 프로덕션 핸들러 안에서 도달 — 단 인자는 전부 `'static` 리터럴이고 13행 전부 DoD 2가 구동해 오타는 테스트가 즉시 잡음) 현상 유지, op 키의 enum 타입화는 M6 후보로 이월. ⑤ P2-4(L6의 `host.reverse` 문단 검사가 §2.5 밖 전체 문서를 뒤지고, 한국어 문서에 무검사 바이트 슬라이스) 수용, §2.5 슬라이스 + `.contains()`로 시정. ⑥ P2-5(스트림 축 seam(`authorize_stream` 호출 2곳)에 열거 앵커 없음 — Body 축만 exhaustive match로 닫힘) 수용, 소스 스캔 pin 테스트로 호출 지점 수·이름을 고정. ⑦ P2-6(핸들러의 `action_of` 경유가 관행이지 구조 아님 — 올바른 리터럴 하드코딩 mutation 생존) 수용, ⑥과 같은 소스 스캔으로 server 프로덕션 코드의 `Action::` 리터럴을 문서화된 예외(`session.attach@data-stream`) 1곳으로 pin. ⑧ P2-7(README 신규 항목 em dash 과밀·X-not-Y 템플릿, testing.md 400자 단문장) 수용, 산문 재작성. **PLAN 정정 2건:** Step 8 (b)의 `tunnel/*` 수정 대상은 무대상이었다(모든 tunnel 게이트가 `server/mod.rs` 소재 — 계획 작성 시점의 위치 추정 오류); 364행의 인가 불요 variant "명시 목록(`Ping`·`Hello`·`SessionEvent`)"은 실제 5종(`Pong`·`Response` 포함)이며 목록의 정본은 코드의 exhaustive 분류 테스트다.
