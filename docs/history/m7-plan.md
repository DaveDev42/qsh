# M7 계획 — Trust UX·profiles·doctor (감사 기록)

M7 실행 계획 원문이다. 2026-09-15에 `PLAN.md`에서 떼어 이리로 옮겼고, 내용은 옮길 당시 그대로다 — 새 판정을 여기에 덧붙이지 않는다. M7의 잔여 두 건은 아직 닫히지 않았다. DoD 1 스톱워치 3회는 `docs/ROADMAP.md` M7 절이 계속 들고 있고, SC7 예약은 같은 문서 M8 절의 DoD 4(wire freeze 후 독립 리뷰 계약)로 넘어갔다.

> **2026-09-02 — M8 선행 착수 (문서 순서 예외).** §5.7은 M7 마감 시점에 이 파일을 M8 계획으로 전면 교체하라고 정한다. 그런데 M7의 잔여는 코드가 아니라 사람이 실행해야 하는 두 항목(DoD 1 스톱워치 3회, SC7 예약)뿐이고, M8의 fuzz DoD는 "parser 타깃당 누적 ≥72 fuzz-hours"라 벽시계 시간이 걸린다. 그 시계를 M7 마감까지 세워두면 리드타임을 두 번 잃는다. 그래서 M7 본문(§1–§5)은 감사 기록으로 그대로 두고 M8 계획을 §6으로 병기한다. 전면 교체는 M7 마감 커밋에서 §1–§5를 삭제하고 §6을 승격하는 것으로 수행한다. 그 분리를 2026-09-15에 수행했다 — §1–§5가 이 파일이 됐고 §6은 M8판 계획으로 정리돼 `docs/history/m8-plan.md`에 있다.
