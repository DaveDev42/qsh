# M2 계획 — 세션 broker + PTY + resume (감사 기록)

2026-09-24에 복원했다. 아래 본문은 M2 실행 계획 원문이다. 커밋 `f5cd30a`(2026-08-19)의 `PLAN.md`에서 그대로 꺼냈고, 같은 날 커밋 `da56b73`가 `PLAN.md`를 M3 계획으로 교체하면서 이 판본은 저장소 작업 트리에서 사라졌다. 새 판정을 덧붙이지 않고 원문 그대로 둔다. 복원 사유는 코드 주석의 옛 계획 인용이 가리키는 문서를 저장소 안에 다시 두기 위함이다.
