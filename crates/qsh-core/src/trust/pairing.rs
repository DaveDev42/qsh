...
struct InviteRecordFile {
    ...
    /// The name `qsh pair invite --as`/`qsh trust invite --as` assigned to
    /// the peer that redeems this invite (ADR-0012 결정 6). Re-validated at
    /// redemption time ([`SharedInviteStore::redeem`]'s own doc) — never
    /// trusted blind just because it parsed off disk.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl InviteStore {
    ...
    pub(crate) fn lock(path: &Path) -> Result<crate::config::FileLock, OpError> {
        if let Some(parent) = path.parent() {
            ...
            // at `qsh init` time, but this call site runs on every invite
            // lock/redeem over a long-running `qsh serve`'s whole
            // lifetime, so sweeping here too bounds how long an orphan
            // can sit around between `qsh init` runs.
    ...
    /// `assigned_name` is `qsh pair invite --as`'s value, stored verbatim
    /// (re-validated at redemption time, never here — see
    /// [`SharedInviteStore::redeem`]'s doc). Returns the RFC 3339
    /// `created_at`/`expires_at` pair.
    pub fn add(
        &mut self,
        secret: &[u8],
        now: SystemTime,
            ...
            .filter_map(|(i, r)| {
                ...
                Some((
                    ...
                    Record {
                        ...
                        created_at,
                        consumed_at,
                        assigned_name: r.assigned_name.clone(),
                    },
                ))
            })
            .collect()
...
pub enum RedeemOutcome {
    ...
    NoMatch,
    /// The matching record's stored `assigned_name` fails
    /// [`super::validate_peer_label`] — a corrupted or hand-edited
    /// `invites.toml`, since `qsh pair invite --as` only ever writes an
    /// already-validated name. Refused **before** `on_matched` runs and
    /// before the record is marked consumed (record left byte-identical),
    /// never falling back to the peer's self-asserted name (ADR-0012 결정
    /// 6: a fallback would hand name authority back to the peer on one
    /// corrupted file).
    InvalidAssignedName,
}
...
impl SharedInviteStore {
    ...
    pub fn redeem(
        ...
        client_proof: &[u8; 32],
        now: SystemTime,
        on_matched: impl FnOnce(Option<&str>) -> bool,
        ...
        // Re-validate a stored `assigned_name` before `on_matched` ever
        // sees it — refuse rather than fall back to the self-asserted name
        // on a corrupted `invites.toml` (`RedeemOutcome::InvalidAssignedName`'s
        // own doc). The record is left untouched either way.
        if let Some(assigned_name) = record.assigned_name.as_deref()
            && super::validate_peer_label(assigned_name).is_err()
        {
            return Ok(RedeemOutcome::InvalidAssignedName);
        }
        if !on_matched(record.assigned_name.as_deref()) {
