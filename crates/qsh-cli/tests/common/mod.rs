...
// under the old names so `adversarial_load.rs`'s existing
// `use common::{..., converged, open_fd_count, poll_stable, rss_kib}` line
// (and any other caller) keeps compiling unchanged. `pub use` (rather than
// a plain fn/struct definition `#![allow(dead_code)]` already covered) is
// its own lint category — `unused_imports`, not `dead_code` — so each
// symbol a given test binary doesn't itself reach needs the allow spelled
// out here instead.
